...
-   **Character Editor**:
    ...
    -   **SillyTavern Tab**: Features a dedicated SillyTavern tab split into "Main" and "Advanced" sub-tabs. Includes a `"Copy data from Online Format"` button (separated by a delimiter in the sub-tab bar) to easily copy/populate core character profile data into the corresponding SillyTavern fields.
...
-   **Features**:
    ...
    -   Collection Tree (recursive rendering of folders).
        -   **Interaction**: Clicking a folder's name selects it to browse its contents in the central explorator view. Double-clicking the folder's name (or clicking the dropdown arrow next to it) expands/collapses the folder in the sidebar tree.
    -   Global Search input.
