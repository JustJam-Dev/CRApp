...
3.  **Self-Healing Rollback Recovery & Visual Alerting:** If the database copy or zip archive extraction encounters any failure:
   ...
   - **Clutter Prevention Cleanup**: Upon a successful rollback and database re-initialization, the pre-import safety backup file is automatically deleted to keep the workspace clean. The backup file is preserved only if the rollback itself or the database initialization fails, as a critical recovery fallback.
